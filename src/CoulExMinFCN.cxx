...
void ThreadTask(std::vector<PointCoulEx*> pcs, const std::vector<double> thetas, const std::vector<int> indx){

	for (int i=0; i<indx.size(); ++i) {
		pcs[indx[i]]->CalculatePointProbabilities(thetas[indx[i]]);
	}
...
void CoulExMinFCN::SetNthreads(int n) {
	nThreads = n;
	threadAlloc.resize(nThreads);
	for (int i=0; i<pointCalcs.size(); ++i) {
		pointCalcs.at(i)->SetThread(i%nThreads);		
		threadAlloc.at(i%nThreads).push_back(i);
	}	 
}
...
double CoulExMinFCN::operator()(const double* par){
	...
	std::vector<double> meanThetas(pointCalcs.size());
	...
	for(unsigned int i=0;i<pointCalcs.size();i++){
		...
		meanThetas.at(i) = exptData.at(i).GetThetaCM();
	}
	if(nThreads > 1){
		...
		for (int t=0; t<nThreads; ++t) {
			Threads[t] = std::thread(ThreadTask, pointCalcs, meanThetas, threadAlloc.at(t));
		}
